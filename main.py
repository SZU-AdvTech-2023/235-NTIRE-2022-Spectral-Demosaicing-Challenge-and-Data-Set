...
from model.Res2Unet_prune import MSFAUnet_base_prune
...
def updateBN(sparse_rate):
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.weight.grad.data.add_(sparse_rate*torch.sign(m.weight.data))  # L1
...
if __name__ == "__main__":
    ...
    model = MSFAUnet_base_prune(8,8,masks_tensor.shape,sigmoid_scale=config['train']['sigmoid_scale'])
    ...
    for epoch in tqdm(range(config['train']['max_epochs']), desc="train epoch: "):
        ...
        for batch in tqdm(train_dataloader, desc="train batch: "):    # batch: (batch_size, C, H, W)
            ...
            loss.backward()
            if config['train']['prune']:
                updateBN(config['train']['prune_rate'])
            optimizer.step()
